...
function removeFirstOccurrences(str, value) {
  ...
  return indexOfOccurrence >= 0 ? str.replace(value, '') : str;
}
...
function removeLastOccurrences(str, value) {
  ...
  const isPresent = indexOfLast !== -1;
  if (isPresent) {
    const firstPart = str.slice(0, indexOfLast);
    const secondPart = str.slice(indexOfLast + value.length);
...
function isPalindrome(str) {
  ...
  const filtered = check.filter((char) => char.match(/[a-z]/));
  for (
    let i = 0, j = filtered.length - 1;
    i < filtered.length / 2;
    i += 1, j -= 1
  ) {
    if (filtered.at(i) !== filtered.at(j)) {
      return false;
    }
  }
  return true;
...
function invertCase(str) {
  ...
  const invertedString = str
    .split('')
    .map((char) => {
      if (char.match(lowercaseLetter)) {
        return char.toUpperCase();
      }
      if (char.match(uppercaseLetter)) {
...
function extractNameFromTemplate(value) {
  ...
  return value.slice(fromIndex + 1, toIndex);
}
...
function unbracketTag(str) {
  const removedFirst = str.replaceAll('<', '');
  const final = removedFirst.replaceAll('>', '');
  return final;
}
...
function encodeToRot13(str) {
  const firstIndexLower = 'a'.charCodeAt();
  const lastIndexLower = 'z'.charCodeAt();
  const firstIndexUpper = 'A'.charCodeAt();
  const lastIndexUpper = 'Z'.charCodeAt();
  const codeRepresentation = str
    .split('')
    .map((ch) => {
      if (ch.match(/[a-z]/)) {
        const decodedNumber =
          ch.charCodeAt() + 13 > lastIndexLower
            ? ch.charCodeAt() + 13 - (lastIndexLower - firstIndexLower + 1)
            : ch.charCodeAt() + 13;
        return String.fromCharCode(decodedNumber);
      }
      if (ch.match(/[A-Z]/)) {
        const decodedNumber =
          ch.charCodeAt() + 13 > lastIndexUpper
            ? ch.charCodeAt() + 13 - (lastIndexUpper - firstIndexUpper + 1)
            : ch.charCodeAt() + 13;
        return String.fromCharCode(decodedNumber);
      }
      return ch;
